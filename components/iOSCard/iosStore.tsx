  ...
  return (
    <>
      {activeGame ? (
        <>
          ...
          <div className="active-game">
            <motion.div layoutId={`animate-${activeGame.title}`} className="inner" ref={ref} style={{ borderRadius: 12 }}>
              <div className="header">
                <motion.img
                  layoutId={`animate-${activeGame.image}`}
                  height={56}
                  width={56}
                  alt=""
                ...
                <div className="header-inner">
                  <div className="content-wrapper">
                    <motion.h2  layoutId={`title-${activeGame.title}`}
                      className="game-title">{activeGame.title}</motion.h2>
                    <motion.p  layoutId={`animate-${activeGame.description}`}
                      className="game-description">{activeGame.description}</motion.p>
                  </div>
      ...
      <motion.ul 
        className="list">
        {GAMES.map((game) => (
          <motion.li layoutId={`animate-${game.title}`}
            key={game.title}
            onClick={() => setActiveGame(game)}
            style={{ borderRadius: 8 }}
          >
            <motion.img
              layoutId={`animate-${game.image}`}
              height={56}
              width={56}
              alt=""
              src={game.image}
            ...
            <div className="game-wrapper">
              <div className="content-wrapper">
                <motion.h2  layoutId={`title-${game.title}`}
                  className="game-title">{game.title}</motion.h2>
                <motion.p   layoutId={`animate-${game.description}`}
                  className="game-description">{game.description}</motion.p>
              </div>
              <button className="button">Get</button>
            </div>
          </motion.li>
